...
// Contact Dropdown Component (scrollable, as previously updated)
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            animate={{ opacity: 1, height: "auto", maxHeight: "200px" }} // Set max height for scrollability
            exit={{ opacity: 0, height: 0 }}
            className="absolute w-full bg-white border-[1px] border-[#6A7368] rounded-[11px] shadow-lg mt-2 overflow-y-auto" // Scrollable with overflow-y-auto
...
// Modal Component (verified profile route)
const Modal = ({ profile, onClose }) => {
  ...
  const handleViewProfile = () => {
    navigate(`/community/profile/${profile.id}`); // Navigate to profile page with profile.id (already correct)
        ...
        >
          ...
          >
            ...
            <div className="flex gap-4 items-center mt-4">
              {/* View Profile Button (verified as correct) */}
              <button
...
const NewBusinesses = () => {
  ...
  const [newProfiles, setNewProfiles] = useState([]); // For "Newly Added"
  const [trendingProfiles, setTrendingProfiles] = useState([]); // For "Trending Businesses"
  ...
  useEffect(() => {
    const fetchProfiles = async () => {
      try {
        ...
        // Fetch Trending Businesses profiles
  ...
  // Extract unique categories and locations from all profiles
  ...
  // Filter profiles based on category and location
  ...
  return (
    <div className="w-full h-fit py-16 flex justify-center bg-[#FFFDF2]">
      <div className="w-[85%] h-fit flex flex-col gap-8">
        ...
        <div className="flex gap-4">
          ...
          <div className="relative w-1/2">
            <button
              ...
              className="w-full border-[1px] border-[#6A7368] text-[#6A7368] rounded-[11px] text-[15px] px-4 py-2 shadow-lg bg-white flex justify-between items-center hover:bg-[#F5F7F5] transition-all duration-300"
            >
            ...
            <AnimatePresence>
              {isCategoryOpen && (
                <motion.div
                  ...
                  style={{ zIndex: 40 }} // Ensure dropdown stays above other content
                >
        ...
        <div className="w-full grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 grid-cols-2 max-[280px]:grid-cols-1 gap-8">
